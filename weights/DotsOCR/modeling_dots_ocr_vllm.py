...
class DotsOCRProcessingInfo(Qwen2_5_VLProcessingInfo):
    ...
    def get_hf_processor(
        ...
        **kwargs: object,
        ...
        # Create image processor directly to avoid circular dependency
        image_processor_kwargs = {}
        if min_pixels is not None:
            image_processor_kwargs["min_pixels"] = min_pixels
        if max_pixels is not None:
            image_processor_kwargs["max_pixels"] = max_pixels
        if size is not None:
            image_processor_kwargs["size"] = size

        image_processor = Qwen2VLImageProcessor(**image_processor_kwargs)
